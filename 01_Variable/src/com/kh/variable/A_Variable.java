...
//		서로 다른 자료형 값 여러개를 출력문 하나에 동시에 출력시 +로 붙혀줘야함.
		System.out.println("노지의 : " + (11000 * 4 * 5) + "원 입니다.");
		System.out.println("안근태 : " + (11000 * 7 * 20) + "원 입니다.");
		System.out.println("노연희 : " + (11000 * 8 * 23) + "원 입니다.");
		
		System.out.println("변수 사용 후");
		int pay = 11000;	//자바에서 =은 대입을 나타내는 연산자. 같다는 == 사용.
		int time = 6;
		int day = 15;
...
//		변수를 사용하여 월급 측정(시급 * 근무시간 * 근무일)
		System.out.println("노지의 : " + (pay * time * day)+ "원 입니다.");
		System.out.println("안근태 : " + (pay * time * day)+ "원 입니다.");
		System.out.println("노연희 : " + (pay * time * (day+5) )+ "원 입니다.");
	...
	public void declareVariable() {
		 ...
		 *3. 해당 영역에 선언된 변수는 해당 영역 안에서만 사용 가능(다른 메소드에서는 불가능.)
		 *	 => 지역변수라는 개념
		 */
		...
		double dNum; // 8byte => 소수점아래 15자리까지 표현 가능.
		// 실수형 중 가장 대표적인 자료형 => 보다 정확한 실수 값을 담을 수 있기 때문에
		...
		isTrue = true;	//true가 보라색인건 자바의 기본값이기 때문.
	...
	public void constantVariable() {
		...
		final int AGE = 20;	//상수란: 변하지 않는 값 => 상수명은 변수명을 전체 대문자로 설정해줘야 함. 상수 앞에는 final 자료입력
