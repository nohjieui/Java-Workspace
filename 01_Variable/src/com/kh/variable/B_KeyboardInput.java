...
public class B_KeyboardInput {
	...
	public void inputTest1() {
		...
		//nextLine() : 사용자가 입력한 값 중 개행이 있을 경우 공백에 무관하게 개행이전까지만 출력해준다.
		//			   특히 사용자가 입격한 값에 공백이 포함 될 여지가 있는 경우 안전하게 nextLine()함수를 쓰자.
		
		
		
		System.out.println("당신의 나이는 몇 살입니까? : ");
		int age = sc.nextInt(); // 사용자가 입력한 값 중 정수형 값을 받을 때 사용하는 메소드
		...
		//000에 사는 키는 000.0cm이며, 나이는 00세인 000님 안녕하세요.
	...
	/*
	 *  정리
	 *  1. 콘솔 창(모니터)에 출력하기 위해 : System.out.print[ln]()a메소드 이용
	 ...
	 *   
	 *   2) '문자'값을 입력받아야 할 경우
	 *      sc.nextLine()메소드를 통해 우선 묹자열을 입력받고, 그 뒤에 .charAt(인덱스)메소드를 통해 문자를 하나만 추출한다.
	 */
}
