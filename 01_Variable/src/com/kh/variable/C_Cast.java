...
public class C_Cast {
	 ...
	 * 
	 * 형변환의 종류
	 * 1. 자동(암시적)형변환 => 자동으로 형변환이 진행되기 때문에 내가 직접 형변환할 필요가 없다.
	 *            => 작은바이트의 자료형 -> 큰바이트의 자료형
	 * 2. 강제(명시적)형변환 => 자동형변환이 되지 않아 내가 직접 강제로 형변환을 해야하는 경우
	 *                  => 큰바이트의 자료평 -> 작은바이트의 자료형
	 * [표현법] (바꿀자료형) 값;
	 * 
	 * (바꿀 자료형) == 형변환연산자 == cast 연산자 라고 다양하게 불림
	 * 
	 * **boolean은 형변환 불가.
	 */
	...
	public void autoCasting() {
		...
		//    아무리 4byte float라고 해도 long형보다 담을 수 있는 값의 범위가 더 크기때문에
		...
		/*
		 * char의 범위 0~65xxx : 각 문자마다 고유의 정수값이 정해져있다. 따라서 쌍방향 변환이 가능하다. 음수값은 오류가 난다.
		 * 참고 : 아스키코드표(0~127, 영문자 대소문자, 숫자, 특수문자)
		 *       유니코드표(0~65xxx, 영어, 숫자, 특수문자, 한글, 그 외 언어)
		 */
		System.out.println((int)'노');
	...
	public void forceCasting() {
		...
		// 해결방법 1. 연산 결과를 int형으로 맞춰서 강제형변환.
		int iSum = (int)(iNum + dNum);
		
		// 해결방법 2. 덧셈 연산전에 dNum변수를 미리 int형으로 강제형변환
		//           자동형변환은 작은 바이트변수가 큰 바이트 변수로 형변환 되기 때문에 큰바이트인 dNum변수를 강제로 int형으로 변환시킨것.
