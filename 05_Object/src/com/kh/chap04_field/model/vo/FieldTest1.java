...
/*
 * 변수구분
 * -전역변수 : 클래스 영역에 바로 선언하는 변수 -> 클래스 내에서만 어디서든 사용 가능
 ...
 *     소멸시점 : 객체 소멸시 같이 소멸
 *     
 *  - 클래스변수 == static변수 : static이라는 예약어가 붙어있냐 아니냐에 따라 멤버변수냐 클래스변수냐로 구분됨.(static 변수는 기울어진 글씨체)
...
public class FieldTest1 { // FieldTest1 클래스 시작
	...
	public void test(int num) { // test 메소드 시작
		...
		/*
		 * 멤버변수 출력시 에러가 안나는 이유
		 * 해당 클래스 객체 생성시 jvm에 값을 해당 필드로 자동으로 대입(초기화)시켜 줬기 때문
		 * 물론, 명시적 초기화도 가능함
		 */
		System.out.println(global);
		
		// 매개변수 출력시 에러 안나는 이유
		// 해당 메서드(test)호출 시 반드시 값을 넘겨줘야 호출이 가능하기 때문에 num에는 반드시 어떤 값이든 초기화가 될것.
