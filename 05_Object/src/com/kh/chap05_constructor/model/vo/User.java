...
public class User {
	 ...
	 *      // 해당 생성자를 통해서 객체 생성 시 실행하고자하느 코드;
	 * }
	 * 
	 * 생성자를 작성하는 목적
	 * 1. 객체를 생성해주기 위한 목적
	 ...
	 * 2. 여러개 생성이 가능하지만 매개변수가 중복되면 안된다.
	 * 4. 매개변수 생성자를 명시적으로 작성하게 되면 기본생성자를 JVM에 자동으로 만들어주지 않는다.
	...
	public User() {
		 ...
		 * JVM이 클래스를 컴파일 할 때 기본생성자가 없는 경우 항상 자동으로 만들어 주기때문.(매개변수있는 생성자가 없는경우)
		 * 단, 매개변수가 있는 생성자가 이미 클래스 안에 작성되어 있는경우 반드시 기본생성자도 함께 작성해줘야한다.
		 * 그러나 기본생성자를 작성하는 습관을 들이자.
		 */
		System.out.println("이거 출력 되나 ?");
	}
	// 4. 매개변수 생성자를 명시적으로 작성
