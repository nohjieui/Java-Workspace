...
public class MethodTest2 {
	...
	public static int method4(int num1) {
		/*
		 * static 메서드 안에서는 static이 아닌 field맴버가 접근이 불가능하다.
		 * 스태틱 메서드는 프로그램 시작시 정적메모리 영역에 들어가게 되는데
		 * 이때 스태틱으로 선언된 필드(전역변수)도 함께 정적메모리 영역에 들어갈 것.
		 * 하지만 스태틱으로 선언되지 않은 전역번수는 스태틱메모리에 올라가는 시점에 사용이 불가능하므로 에러가 발생함.
		 */
