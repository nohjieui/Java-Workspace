...
public class PolyRun {
	...
	public static void main(String[] args) {
		...
		Child1 c1 = new Child1(); // 기본생성자에 슈퍼생성자 호출하는 구문이 항상있음. 따라서 부모객체사용 가능.
		c1.printChild1();
		((Parent)c1).printParent();
		// c1 레퍼런스로 Parent(부모), Child(자식)에 있는 필드나 메서드 둘 다 접근가능.
		// c1 레퍼런스변수가 Parent에 있는 메소드에 접근시 자동으로 형변환된채 진행된.
		// 즉 자식에서 부모클래스로 자동형변환(업캐스팅) -> 작은것에서 큰것으로 형변환 된것과 같은 이치
		 ...
		 * Parent형으로 '자동형변환'이 되고 있기때문에
		 * 상속 구조에서는 클래스 형변환이 가능하다.
		 */
		p2.printParent(); // p2라는 레퍼런스 변수로는 Parent에만 접근이 가능.
		((Child1) p2).printChild1(); // 단, 자식객체로 접근하고 싶을때 Child형으로 강제형변환 후 Child에 접근가능.
		 ...
		 * 
		 * 다형성을 쓰는 이유
		 *  - 부모타입으로부터 파생된 여러가지 타입의 자식객체들을 부모클래스 타입하나로 다룰 수 있음.
		 */
		...
		// 단, 다형성이 적용되면 부모타입레퍼런스 변수로 모든 자식객체를 다받아줄 수 있다.
		...
		for(int i = 0; i < arr.length; i++) {
			...
			// 현재 참조하고있는 레퍼런스 변수가 실제로 어떤 클래스(객체)타입을 참조하고 있는지 확인 할 때 사용
...
//			}
			...
			arr[i].print();
			/*
			 * * 동적바인딩 : 프로그램이 실행되기전에 전에 자동으로 컴파일 되면서 정적으로 현재 레퍼런스 변수의 메소드를 가리키게됨.
			 * 단, 참조하고 있는 자식클래스에 해당 메소드가 오버라이딩 되어 있다며
			 * 프로그램을 시작할 때는 동적으로 그 자식클래스의 오버라이딩 된 메소드를 찾아서 실행하게됨
			 * (ctrl 누르고 들어가보면 Parent클래스의 메소드를 실행한다고 했지만(정적바인딩) ,
			 * 실제로 실행시에는 자식클래스의 오버라이딩 된 메소드가 진행됨.(동적바인딩))
			 */
