...
public interface Basic {
	 ...
	 * - 무조건 구현해야 하는게 있을때 인터페이스를 만들어 상속하게 된다.
	 * - 인터페이스는 다중 상속을 허용한다.
	 *   (결국 메소드에 대한 정의만 하고있어서 메소드가 겹치더라도 최종 구현 부분은 구현 클래스에서 이루어질것이기 때문에
	 *    다중상속(구현)이 가능함.)
	 ...
	 *    인터페이스는 클래스의 기능(함수) 구현을 강제하기 위해 사용된다. 
	 *    즉 구현을 강제함으로써 구현객체의 동일한 동작을 보장할 수 있다.
	 *    
	 * extends 와 implements
	 *  - 클래스간에 상속 관계일 때 : 클래스명 extends 부모클래스명
	 *  - 클래스와 인터페이스의 구현관계일 때 : 클래스명 implements 인터페이스명
