...
public class A_UncheckdeException {
	/*
	 * RuntimeException
	 * - 프로그램 실행시 발생되는 예외들
	 * 
	 ...
	 * - NagativeArraySizeException : 배열의 크기를 음수로 지정할 경우 발생하는 예외
	...
	public void method1() {
		...
		// ArithmeticException : 나누가 연산시 0으로 나눌 때 발생하는 예외.
...
//		System.out.println("프로그램 종료.");
		...
		try {
			...
			System.out.println("0으로 나눌 수 없습니다.");
			//e.printStackTrace(); // 현재발생한 오류를 추적할 수 있는 로그정보 제공
		}
		System.out.println("프로그램 종료");
	}
	...
	public void method3() {
		...
		int size = Integer.parseInt(sc.nextLine()); // 문자열을 입력받아 입력받은 문자를 int형으로 변경
		
//		if(size > 0) { // 방법 1. if문으로 size값이 양수값일때만 생성하도록 제어
//			int[] arr = new int[Math.abs(size)]; // 방법 2. 항상 양의 정수 값이 들어갈수 있도록 Math.abs메서드 사용
//		}
...
//		} // 다중 catch블럭이 줄어들었음. 하지만 정확히 어떤 에러인지 알 수 없음.
	 ...
	 * - 예외처리 구문으로도 해결 가능 => 예외가 발생했을 때를 대비해서 그 때 실행할 내용을 정의해 두는 것.
	 * 
	 * 예측이 가능한 상황 => 조건문으로 해결(권장하는 방법)
	 * 예측이 불가능한 상황 => 예외처리 구문으로 해결
	 * 
	 * RuntimdException 계열은 충분히 예측 가능한 상황이기 때문에 조건문으로 해결하는 것을 권장
	 * 
