...
public class ListRun {
	 ...
	 * - 컬렉션의 장점
	 * 1. 크기에 제약이 없다.
	 *    => 크기지정을 해줄필요도 없고 만일 크기지정을 해놔도 알아서 크기가 늘어나면서 새로운 데이터를 추가해주는
	 *       코드가 이미 정의되어 있음.
	 ...
	 * 컬렉션의 3가지 분류
	 * 
	 * List계열 => 담고자 하는 값(value)만 저장 / 값 저장시 순서 유지됨(index의 개념이 있음) / 중복값 허용.
	 *            ex) ArrayList, Vector, LinkdList => ArrayList가 제일 많이 쓰임(90% 이상)
	 * Set계열 => 담고자 하는 값(value)만 저장 / 값 저장 시 순서유지 x(index개념이 없음) / 중복값 허용 x
	 * 	         ex) HashSet, TreeSet
	 * Map 계열 => 키(key) + 담고자 하는 값(value)세트로 저장 / 값 저장시 순서유지  x(index 개념 없음)
	 *           / key 중복 허용 x
	...
	public static void main(String[] args) {
		...
		list.add(new Music("Cookie", "new jeans")); // list에는 Music 객체만 들어가도록 제네릭 설정을 해둠
		list.add(new Music("hype boy", "new jeans"));
		list.add(new Music("nude", "idle"));
		//list.add("히히"); // list에는 Music 객체만 들어가도록 제네릭 설정을 해두어서 입력불가
		/*
		 * 순서가 유지되면서 값이 추가됨.
		 * 각 index에 담겨있음.
		 * 크기에 제약이 없음. ArrayList<Music> list = new ArrayList<>(3);에 3칸짜리 배열로 설정했음에도 더 추가가 가능함
		 * 다양한 타입의 값 추가 가능.
		 */
		System.out.println(list);
		...
		list.set(0, new Music("Butter", "BTS"));
		
		System.out.println(list);
		
		// 4. remove(int index) : 리스트에 해당 인덱스 위치에 있는 값을 삭제시켜주는 메소드.
		list.remove(1);
		...
		// 6. get(int index) : 반환형 E => 리스트에 해당 index위치의 데이터를 반환해주는 메소드
		// 제네릭 설정 전
		//Music m = (Music) list.get(0); // 다형성 => 강제형변환 시켜줘야함.
