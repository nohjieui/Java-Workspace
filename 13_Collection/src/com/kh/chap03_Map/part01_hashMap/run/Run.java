...
public class Run {
	...
	public static void main(String[] args) {
		...
		// key값 : String, value값 : Snake
		
		/*
		 * Collection의 계층구조를 보면 List나 Set 계열은 Collection을 구현한 클래스 => 데이터를 추가할 때 add메소드
		...
		Snack s = hm.get("칸초"); // 제네릭설정을 해놨기 때문에 Snack타입 객체로 강제형변환 하는 과정이 생략되었다.
		                         // HashMap<String, Snack> hm 에서 value값을 Snack만 들어갈 수 있도록 설정해두었기 때문.
		...
		hm.replace("포테이토칩", new Snack("많이짠맛", 1500));
		System.out.println(hm);
		//.put 과 .replace의 차이점 : .put => key값에 다른값(ex 포테이토칩2)이 들어가면 추가가 됨.
		//                       : .replace => 해당 key값(ex 포테이토칩2)이 없을 경우 추가가 안됨.
		...
		//  2) 1)의 과정에서 작업된 keySet을 Iterator에 담기
		...
		//  2) entrySet 에 있는 것들을 Iterator에 담기.
