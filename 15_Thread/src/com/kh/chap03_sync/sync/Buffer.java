...
public class Buffer {
	 ...
	 * - 동기화 메소드는 메소드 선언에 synchronized 키워드를 붙이고 인스턴스, 정적메소드 어디서든 사용 가능하다.
	 * - 동기화 메소드는 스레드가 메소드를 실행하면 메소드 전체에 즉시 락을 걸고 메소드가 종료되면 락이 풀린다.
	 * - 메소드 전체가 아니라 일부내용만 락을 걸고 싶다면 동기화 블럭을 만들면 된다. (synchronized(공유객체){})
	 */
	public synchronized void getData() { // Consumer를 호출
	...
	public synchronized void setData(int data) { //Producer를 호출
