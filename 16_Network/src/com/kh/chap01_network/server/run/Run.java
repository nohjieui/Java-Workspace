...
public class Run {
	 ...
	 * - 데이터를 교환하기에 앞서서 서버, 클라이언트 연결되있어야됨.(서버가 먼저 실행되면서 클라이언트의 요청을 기다림)
	 * - 신뢰성 있는 데이터 전달 가능.
	 * 
	 * SeverSocket
	 ...
	 * - 메소드 : 
	 *  > Socket client = server.accept() 메소드를 통해 클라이언트의 연결은 기다리고 연결이 오면 Socket객체로 반환
	 *  
	 *  Socket
	 ...
	 *   > InputStream in = client.getInputStream() 메소드를 통해 데이터를 "읽을수있는" 입력스트림을 생성.
	 */
	public static void main(String[] args) {
		new Server().serverStrart();
	}
